...
def get_items_with_min_support(item_set, all_transactions, min_support,
    ...
    for item, count in local_set.items():
        if (count/float(N)) >= min_support:  #check the value and if value is at least min_support add the key to rtn
            rtn.add(item)
...
def get_support(set_count_map, item, N):
    ...
    return set_count_map[item]/float(N) #return the relative support
...
def join_set(s, l):
    ...
    for set1 in s:
        for set2 in s:
            if set1 is not set2:  #as long as the two sets in s are not the same set then make the union and test the length
                unionSet = set1.union(set2)
                if len(unionSet) == l:
                    finalSets.add(unionSet)
    return finalSets
